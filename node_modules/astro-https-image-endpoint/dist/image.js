...
const isRemotePath = (src) => /^(http|ftp|https|ws):?\/\//.test(src) || src.startsWith('data:');
const isRemoteAllowed = (src, { domains = [], remotePatterns = [], }) => {
    if (!isRemotePath(src))
        return false;
...
export const GET = async ({ request }) => {
    ...
    if (!('transform' in imageService)) {
        throw new Error(`${PACKAGE_NAME} Configured image service is not a local service`);
    }
    ...
    const filePath = (os.platform() === 'win32'
        ...
        : transform.src.replace(/^\/@fs/, '')).replace(/\?(.*)$/, '');
